...
export default function UsersArchivePage() {
  ...
  const handlePermanentDelete = async () => {
    ...
    try { await api.delete(`/users/${deletingId}/permanent`); setDeletingId(null); refetch(); } catch (e) { setActionError(getApiErrorMessage(e)); }
            ...
            {isLoading ? (<tr><td colSpan={5} className="text-center py-8 text-text-muted"><Loader2 className="inline animate-spin mr-2" size={16} />Loading...</td></tr>) : isError ? (<tr><td colSpan={5} className="text-center py-8 text-accent-red">{getApiErrorMessage(error)}</td></tr>) : filtered.length === 0 ? (<tr><td colSpan={5} className="px-4 py-12"><div className="border-l-4 border-accent-orange pl-4"><p className="text-accent-orange font-medium">No archived users found.</p></div></td></tr>) : filtered.map((user, idx) => (
              <tr key={user.id} className="border-b border-card-border transition">
                ...
                <td className="px-4 py-3 flex items-center gap-2">
                  ...
                  <button onClick={() => { setDeletingId(user.id); setDeletingName(`${user.firstName} ${user.lastName}`); }} className="inline-flex items-center gap-1 rounded-lg bg-accent-red/10 px-2.5 py-1 text-sm font-medium text-accent-red hover:bg-accent-red/20 transition-colors" title="Delete permanently"><Trash2 size={14} /></button>
                </td>
              </tr>
            ))}
          </tbody></table></div>
      </div>
      {deletingId && (<div className="fixed inset-0 z-50 flex items-center justify-center"><div className="absolute inset-0 bg-black/60" onClick={() => setDeletingId(null)} /><div className="relative bg-card-bg border border-card-border rounded-lg shadow-xl w-full max-w-sm mx-4 p-6 space-y-4"><h3 className="text-lg font-bold text-text-primary">Permanently Delete</h3><p className="text-sm text-text-secondary">This action cannot be undone. The account for <strong className="text-text-primary">&ldquo;{deletingName}&rdquo;</strong> will be permanently removed and cannot be recovered.</p><p className="text-sm text-text-secondary">Are you sure you want to proceed?</p><div className="flex justify-end gap-2 pt-2"><button onClick={() => setDeletingId(null)} className="px-4 py-2 text-sm text-text-secondary border border-card-border rounded-lg hover:bg-white/5 transition">Cancel</button><button onClick={handlePermanentDelete} disabled={deleteLoading} className="px-4 py-2 text-sm text-white bg-red-600 rounded-lg hover:bg-red-700 transition disabled:opacity-60">{deleteLoading ? <Loader2 size={14} className="inline animate-spin" /> : 'Delete Permanently'}</button></div></div></div>)}
    </div>
  );
}
